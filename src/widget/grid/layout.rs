...
pub(super) fn layout<Message, Theme, Renderer>(
    ...
    row_count: usize,
    element_count: usize,
    rows: &mut [GridRow<'_, Message, Theme, Renderer>],
    ...
    Renderer: renderer::Renderer,
{
    ...
    for row in rows {
        ...
        for (col_idx, element) in row.elements.iter_mut().enumerate() {
    ...
    Renderer: renderer::Renderer,
{
    ...
    for (row_position, (row, &row_height)) in rows.iter_mut().zip(row_heights).with_position() {
        let mut x = padding.left;
        for (col_position, (element, &column_width)) in
            row.elements.iter_mut().zip(column_widths).with_position()
        {
